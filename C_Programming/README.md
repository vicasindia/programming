### C Programming

#### 01. Introduction

* [Welcome](hello.c)
* [Format Specifiers](format_specifiers.c)
* [Copy Input to Output](copy_input_to_ouptput.c)

#### 02. Data types, Operators and Expressions

#### 03. Flow Control

* [if statement](if_statement.c)
* [if-else statement](if-else_statement.c)
* [if else-if statement](if_else-if_statement.c)
* [Nested if statement](nested_if_statement.c)
* [Even Odd](even_odd.c)
* [Largest among 3 numbers](largest_number_among_three.c)
* [Largest among 3 numbers (using nested loop)](largest_number_among_three_02.c)
* [switch statement](switch_statement.c)
* [Simple Calculator](simple_calculator.c)
* [while loop](while_loop.c)
* [do while loop](do_while_loop.c)
* [for loop](for_loop.c)
* [Read Input until user enters a positive number](read_input_until_enter_positive_number.c)
* [continue](continue.c)
* [break](break.c)
* [Digits Counter](count_digits.c)
* [Palindrome Number](palindrome_number.c)

#### 04. Functions

* [sum()](sum_function.c)
* [diff()](diff_function.c)
* [product()](product_function.c)
* [division()](division_function.c)
* [remainder()](remainder_function.c)
* [max()](max_function.c)
* [min()](min_function.c)
* [avg()](avg_function.c)
* [pow()](pow_function.c)
* [sqrt()](sqrt_function.c)
* [cbrt()](cbrt_function.c)
* [kthroot()](kthroot_function.c)
* [mean()](mean_function.c)
* [mode()](mode_function.c)
* [median()](median_function.c)
* [deviation()](deviation_function.c)
* [countDigits()](count_digits_function.c)
* [first_index()](first_index_function.c)
* [last_index()](last_index_function.c)
* [strlen()](strlen_function.c)
* [strcpy()](strcpy_function.c)
* [strcmp()](strcmp_function.c) 
* [strcat()](strcat_function.c)
* [atoi()](atoi_function.c)
* [itoa()](itoa_function.c)
* [isUpper()](is_upper_function.c)
* [isLower()](is_lower_function.c)
* [toUpperCase()](to_upper_case_function.c)
* [toLowerCase()](to_lower_case_function.c)
* [isAlphaNumeric()](is_alpha_numeric_function.c)
* [isNumeric()](is_numeric_function.c)
* [getAlphaNumeric()](get_alpha_numeric_function.c)
* [caseToggle()](case_toggle_function.c)
* [isSpace()](is_space_function.c)

#### 05. Pointers

* [Pointer Intro](pointer_intro.c)
* [Address Arithmetic](pointer_arithmetic.c)

#### 06. Arrays

#### 07. Strings

#### 08. Input Output

#### 09. Recursion

#### 10. Maths

* [Natural Numbers](natural_numbers.c)
* [Tables](tables.c)
* [Smallest and Largest Number](min_max_number.c)
* [Largest among 4 numbers](max_of_four.c)
* [Second largest among 3 numbers](second_largest_among_three.c)
* [Sum of Natural Numbers: O(n)](sum_of_natural_numbers.c)
* [Sum of Natural Numbers: O(1)](sum_of_natural_numbers_02.c)
* [Sum of first & last digit of an integer](first_last_digit_sum.c)
* [Fibonacci Numbers](fibonacci.c)
* [Fibonacci using Recursion](fibonacci_using_recursion.c)
* [Factorial!](find_factorial.c)
* [Factorial using Recursion](factorial_using_recursion.c)
* [Prime Number](check_prime_number.c)
* [Perfect Number](perfect_number.c)
* [Armstrong Number](armstrong_number.c)
* [Circle](circle.c)
* [Triangle](triangle.c)
* [Rectangle](rectangle.c)

#### 11. Other Programs

* [ASCII Table ★](ascii_table.c)
* [Anagrams](anagrams.c)
* [Decimal to Binary Converter](binary_converter.c)
* [Celsius to Fahrenheit Converter](celsius_to_fahrenheit.c)
* [Fahrenheit to Celsius Converter](fahrenheit_to_celsius.c) 
* [Char Sum](char_sum.c)
* [Digit Frequency](digit_frequency.c)
* [English Alphabet](english_alphabet.c)
* [Fast Input ★](fast_input.c)
* [Leap Year (366 days)](leap_year.c)
* [Minimum numbers of Notes](minimum_number_of_notes.c)
* [Number to Word or Print Even-Odd](number_word_or_even_odd.c)
* [Palindrome](isPalindrome.c)
* [Palindrome (String)](check_palindrome.c)
* [Palindrome using Functions](check_palindrome_using_functions.c)
* [Printing Tokens](printing_tokens.c)
* [Swap two numbers](swap_two_numbers.c)
* [Swap without third variable](swapping_without_third_variable.c)
* [Swap using Pointers](swap_using_pointers.c)
* [Time Conversion](time_conversion.c)
* [Toggle String](toggle_string.c)
* [Validate ISBN](validate_isbn.c)
* [Version Control System](version_control_system.c)
* [Vowels Counter](vowels_counter.c)
* [Vowels Counter using Function](vowels_counter_02.c)
